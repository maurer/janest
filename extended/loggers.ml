    ...
    ksprintf (fun str ->
     if may_log lev then
        ...
        Unix_utils.wrap_block_signals (fun () ->
          Mutex.critical_section mtx ~f:(fun () ->
...
module MakeChannelSpec (ChannelSpec : CHANNEL_SPEC) : SPEC = struct
  ...
  let max_lev_len =
    ...
    Array.fold_left ~f:coll ~init:0 all_levs + 1
  ...
  let print_repeated () =
    ...
    fprintf ppf "%s  DFS-%s:%slast message repeated %d times\n%!"
      last_msg_time str_lev spaces last_msg_count
  ...
  let logger lev msg =
    ...
    if lev_msg = !last_msg_ref then
      ...
      fprintf ppf "%s  DFS-%s:%s%s\n%!" time str_lev spaces msg
end

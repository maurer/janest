...
let unix_error = Core.Unix_ext.unix_error

exception Timeout

let h_name_of_sockaddr = function
  | ADDR_INET (inet_addr, _) -> (gethostbyaddr inet_addr).h_name
...
let h_name_or_string_of_sockaddr = function
  | ADDR_INET (inet_addr, _) ->
      (try (gethostbyaddr inet_addr).h_name
...
let tcp_socket () = socket PF_INET SOCK_STREAM 0

let connect_tmout s sockaddr con_timeout =
  set_nonblock s;
  try connect s sockaddr; true
  with Unix_error (EINPROGRESS, _, _) ->
    match select [s] [s] [] con_timeout with
    | [], [], [] -> false
    | _ ->

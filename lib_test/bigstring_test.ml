...
let repr bs = 
  if Bigstring.length bs > 30 then
    ...
    Bigstring.blit_bigstring_string bs s ~src_pos:0 ~dst_pos:0 ~len:30;
...
let blit_test ~n ~src_pos ~dst_pos ~len (s1,s2) = 
  ...
  let s_result = 
    try 
      ...
      `Success s2
    with 
      e -> `Failure (exn_to_string e)
  in
  let bs_result = 
    try 
    let bs1 = Bigstring.of_string s1 in
    let bs2 = Bigstring.of_string s2 in
      Bigstring.blit ~src_pos ~dst_pos len ~src:bs1 ~dst:bs2;
...
let simple_conversion_test ~n s = 
  ...
  let bs = Bigstring.create len in
  Bigstring.blit_string_bigstring s bs ~src_pos:0 ~dst_pos:0 ~len;
  let s' = String.create len in
  Bigstring.blit_bigstring_string bs s' ~src_pos:0 ~dst_pos:0 ~len;
  (sprintf "%s: %s" n s) @? (s' = s)
;;
...
let fdpair_test ~n fdpair sender receiver bs =
  try
    ...
    let sth = Thread.create 
      (fun () -> 
        try sender bs write
        with e -> eprintf "ERROR: %s" (exn_to_string e)) 
      ()
    in
    receiver ~n bs read;
    Thread.join sth;
    ...
    Unix.close write
  with
    e -> assert_failure (sprintf "%s: receive exception: %s" n (exn_to_string e))
...
#else
#warning "MSG_NOSIGNAL not supported, wont test bigstring::really send/recv"
#endif
